...
@section('content')
        ...
        <div class="p-4 lg:p-8">
            <!-- Profile Header -->
            <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl p-8 mb-6 text-white">
                <div class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                    <div class="relative">
                        <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center text-primary-600 text-3xl font-bold shadow-lg">
                            @if($user->avatar)
                                <img id="profileImage" src="{{ asset('public/' . $user->avatar) }}" alt="Profile" class="w-24 h-24 rounded-full object-cover">
                            @else
                                <img id="profileImage" src="" alt="Profile" class="w-24 h-24 rounded-full object-cover hidden">
                                <span id="profileInitials">{{ strtoupper(substr($user->first_name ?? $user->name, 0, 1)) }}{{ strtoupper(substr($user->last_name ?? '', 0, 1)) }}</span>
                            @endif
                        </div>
                        <button onclick="openImageUpload()" class="absolute bottom-0 right-0 bg-orange-500 hover:bg-orange-600 text-white p-2 rounded-full shadow-lg transition-colors">
...
<div class="text-center md:text-left flex-1">
    <h1 class="text-3xl font-bold mb-2">{{ $user->name ?? ($user->first_name . ' ' . $user->last_name) }}</h1>
    <p class="text-primary-100 mb-3">{{ $user->email }}</p>
    <div class="flex flex-wrap justify-center md:justify-start gap-3">
        ...
        @else
            <button onclick="resendVerification()" id="resendBtn" 
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-500 hover:bg-red-600 text-white transition-colors">
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white bg-opacity-20 text-white">
            <i class="fas {{ $user->login_type === 'google' ? 'fa-google' : 'fa-sign-in-alt' }} mr-2"></i>
            {{ $user->login_type === 'google' ? 'Google Login' : 'Email Login' }}
        </span>
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium 
            {{ $user->status === 'active' ? 'bg-green-500' : 'bg-red-500' }} text-white">
            <i class="fas {{ $user->status === 'active' ? 'fa-check' : 'fa-times' }} mr-2"></i>
            {{ ucfirst($user->status) }}
        </span>
    </div>
</div>
                    ...
                    <div class="text-center">
                        <p class="text-primary-100 text-sm">Last Login</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
                <div class="border-b border-gray-200">
                    <nav class="flex space-x-8 px-6">
                        <button class="tab-btn active py-4 px-2 border-b-2 border-primary-600 text-primary-600 font-medium text-sm" data-tab="personal">
                            <i class="fas fa-user mr-2"></i>Personal Info
                        </button>
                        <button class="tab-btn py-4 px-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-medium text-sm" data-tab="security">
                            <i class="fas fa-shield-alt mr-2"></i>Security
                        </button>
                        <button class="tab-btn py-4 px-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-medium text-sm" data-tab="preferences">
                            <i class="fas fa-cog mr-2"></i>Preferences
                        </button>
                        <button class="tab-btn py-4 px-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-medium text-sm" data-tab="activity">
                    ...
                    <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="email" name="email" value="{{ old('email', $user->email) }}" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 pr-12 @error('email') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="file" name="avatar" accept="image/*" 
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('avatar') border-red-500 @enderror">
                            <p class="text-sm text-gray-500 mt-1">Upload a new profile picture (JPG, PNG, max 2MB)</p>
                            @error('avatar')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            <button type="button" onclick="window.location.reload()" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                                Cancel
                            </button>
                            <button type="submit" class="px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-lg">
                                Save Changes
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-blue-50 rounded-lg p-6">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h3 class="text-lg font-semibold text-blue-800">Google Account</h3>
                                    <p class="text-blue-600">You're signed in with Google. Password changes should be made through your Google account.</p>
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="bg-yellow-50 rounded-lg p-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-yellow-800">Email Verification</h3>
                                    <p class="text-yellow-600 text-sm">Your email address is not verified. Please verify to secure your account.</p>
                                </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between p-4 bg-white rounded-lg border">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <p class="font-medium text-gray-800">Last Profile Update</p>
                    ...
                    <form method="POST" action="{{ route('user.preferences.update') }}" class="space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-6">
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="font-medium text-gray-800">Search Results</p>
                                        <p class="text-sm text-gray-600">Receive notifications when new search results are available</p>
                                    </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="font-medium text-gray-800">Weekly Summary</p>
                                        <p class="text-sm text-gray-600">Get a weekly summary of your search activity</p>
                                    </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="font-medium text-gray-800">Marketing Updates</p>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-blue-100 text-sm">Total Searches</p>
                                        <p class="text-3xl font-bold">{{ $stats['total_searches'] ?? 0 }}</p>
                                    </div>
                            ...
                            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-6 text-white">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-green-100 text-sm">Leads Saved</p>
                                        <p class="text-3xl font-bold">{{ $stats['saved_leads'] ?? 0 }}</p>
                                    </div>
                            ...
                            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-purple-100 text-sm">Account Age</p>
                                        <p class="text-3xl font-bold">{{ $user->created_at->diffForHumans(null, true) }}</p>
                                    </div>
                        ...
                        <!-- Recent Activity -->
                        <div class="bg-gray-50 rounded-lg p-6">
                            <h3 class="text-lg font-semibold text-gray-800 mb-4">Recent Saved Leads</h3>
                            @if($recentLeads && $recentLeads->count() > 0)
                                    ...
                                    @foreach($recentLeads->take(5) as $lead)
                                        <div class="flex items-center space-x-4 p-4 bg-white rounded-lg">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-sm text-gray-600">{{ $lead->address }} • Saved {{ $lead->created_at->diffForHumans() }}</p>
                                            </div>
                                            <div class="text-right">
                                                @if($lead->rating)
                                                    <div class="flex items-center text-yellow-500">
                                                        <i class="fas fa-star text-xs mr-1"></i>
                                                        <span class="text-sm">{{ $lead->rating }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                @if($recentLeads->count() > 5)
                                    <div class="mt-4 text-center">
                                        <a href="{{ route('user.leads') }}" class="text-primary-600 hover:text-primary-700 font-medium">
                            ...
                            @else
                                <div class="text-center py-8">
                                    ...
                                    <p class="text-gray-500">No saved leads yet</p>
                                    <a href="{{ route('user.dashboard') }}" class="text-primary-600 hover:text-primary-700 font-medium mt-2 inline-block">
            ...
            <div class="bg-red-50 border border-red-200 rounded-xl p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-4 bg-white rounded-lg border border-red-200">
                        ...
                        <button onclick="confirmAccountDeletion()" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm">
                            Delete Account
                        </button>
                    </div>
                    <div class="flex items-center justify-between p-4 bg-white rounded-lg border border-red-200">
                        <div>
                            <p class="font-medium text-gray-800">Clear All Data</p>
                            <p class="text-sm text-gray-600">Remove all search history and saved leads</p>
                        </div>
                        <button onclick="confirmDataClear()" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm">
                            Clear Data
                        </button>
                    </div>
                </div>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 mb-6">Are you sure you want to delete your account? This action cannot be undone and will permanently delete all your data.</p>
                    <form method="POST" action="{{ route('user.account.delete') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="deleteConfirmation" class="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="DELETE">
                        </div>
                        <div class="flex space-x-3">
                            <button type="button" onclick="closeDeleteModal()" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                                Cancel
                            </button>
                            <button type="submit" id="confirmDeleteBtn" disabled class="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">
                                Delete Account
                            </button>
                        </div>
                    </form>
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6">
                    <div class="flex items-center mb-4">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800">Clear All Data</h3>
                    </div>
                    <p class="text-gray-600 mb-6">This will permanently delete all your search history and saved leads. Your account will remain active.</p>
                    <form method="POST" action="{{ route('user.data.clear') }}">
                        @csrf
                        @method('DELETE')
                        <div class="flex space-x-3">
                            <button type="button" onclick="closeClearDataModal()" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                                Cancel
                            </button>
