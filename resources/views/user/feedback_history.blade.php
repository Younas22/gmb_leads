...
<main class="p-4 lg:p-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Your Feedback History</h1>
                <p class="text-gray-600">Track all the feedback you've shared with us</p>
            </div>
            <button onclick="openFeedbackModal()" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">Total Feedback</p>
                    <p class="text-3xl font-bold text-gray-800">{{ $stats['total'] }}</p>
                </div>
                <div class="bg-primary-100 rounded-lg p-3">
                    <i class="fas fa-comment-alt text-primary-600 text-xl"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ number_format($stats['avg_rating'], 1) }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 mb-6">
        <form method="GET" action="{{ route('user.feedback.history') }}" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-48">
                <label class="block text-sm font-medium text-gray-700 mb-1">Filter by Type</label>
                <select name="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm">
            ...
            <div class="flex-1 min-w-48">
                ...
                <select name="rating" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm">
            ...
            <a href="{{ route('user.feedback.history') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2 rounded-lg text-sm font-medium transition-colors border">
                Clear
            </a>
        </form>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-start space-x-4">
                    <div class="bg-{{ $item->feedback_type == 'bug' ? 'red' : ($item->feedback_type == 'feature' ? 'green' : ($item->feedback_type == 'suggestion' ? 'yellow' : 'blue')) }}-100 rounded-lg p-3">
                        <i class="fas fa-{{ $item->feedback_type == 'bug' ? 'bug' : ($item->feedback_type == 'feature' ? 'plus-circle' : ($item->feedback_type == 'suggestion' ? 'lightbulb' : 'comment')) }} text-{{ $item->feedback_type == 'bug' ? 'red' : ($item->feedback_type == 'feature' ? 'green' : ($item->feedback_type == 'suggestion' ? 'yellow' : 'blue')) }}-600 text-xl"></i>
                    </div>
                    <div>
                        <div class="flex items-center space-x-3 mb-2">
                            <h3 class="text-lg font-semibold text-gray-800">{{ $item->feedback_type_name }}</h3>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @endif
                    <button onclick="toggleFeedback({{ $item->id }})" class="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <!-- Expandable Content -->
            <div id="content-{{ $item->id }}" class="hidden border-t border-gray-200 pt-4">
                <div class="bg-gray-50 rounded-lg p-4 mb-4">
                    <h4 class="text-sm font-medium text-gray-700 mb-2">Your Feedback:</h4>
                    <p class="text-gray-800 text-sm leading-relaxed">{{ $item->message }}</p>
                </div>
                ...
                <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <p class="text-blue-800 text-sm leading-relaxed">{{ $item->admin_response }}</p>
                    <p class="text-xs text-blue-600 mt-2">Responded on {{ $item->updated_at->format('M d, Y \a\t g:i A') }}</p>
                </div>
                @else
                <div class="bg-orange-50 rounded-lg p-4 border border-orange-200">
                    <p class="text-orange-700 text-sm flex items-center">
        ...
        <div class="bg-white rounded-xl shadow-sm p-8 border border-gray-100 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-500 mb-2">No Feedback Found</h3>
            <p class="text-gray-400 mb-4">You haven't submitted any feedback yet.</p>
            <button onclick="openFeedbackModal()" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
